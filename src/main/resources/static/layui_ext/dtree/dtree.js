...
layui.define(['jquery', 'layer', 'form'], function (exports) {
    ...
    var LI_NAV_CHILD = "dtree-nav-ul-sid", LI_NAV_ITEM = "dtree-nav-item",
        ...
        LI_NAV_FIRST_LINE = "dtree-nav-first-line", LI_NAV_LINE = "dtree-nav-line",
        LI_NAV_LAST_LINE = "dtree-nav-last-line";
...
//			level: "level",			  //层级
    ...
    DTree.prototype.setting = function (options) {
        ...
        this.success = this.options.success || OPTIONS.success || function (data, obj, first) {
        }; //树加载完毕后执行解析树之前的回调
        this.done = this.options.done || OPTIONS.done || function (data, obj, first) {
        };	 //树加载完毕后的回调
        this.formatter = $.extend(this.formatter, this.options.formatter || OPTIONS.formatter) || this.formatter; //数据过滤
        this.error = this.options.error || OPTIONS.error || function (XMLHttpRequest, textStatus, errorThrown) {
        }; // 异步加载异常回调
        this.complete = this.options.complete || OPTIONS.complete || function (XMLHttpRequest, textStatus) {
        };	// 异步加载完成回调
    ...
    DTree.prototype.loadTree = function (listData, level, $ul) {
        ...
        if (listData) {
            ...
            for (var i = 0; i < listData.length; i++) {	// 遍历跟节点或追加的跟节点
                var data = listData[i];
                if (typeof data !== "object") continue;
                if (data[_this.response.treeId] == data[_this.response.parentId]) {
                    _this.errData.push(data);
                }
                var parseData = _this.parseData(data);
    ...
    DTree.prototype.replaceDom = function ($div, treeId, last, spread, disabled, hide) {
        ...
        return {
            fnode: function (ficonClass) {	// + - 图标
                ...
                if (fnode != "") {
                    _this.getNodeDom($div).fnode().replaceWith($(fnode));
                }
            },
            node: function (iconClass) {	// 二级图标样式
                ...
                if (snode != "") {
                    _this.getNodeDom($div).snode().replaceWith($(snode));
                }
            },
        ...
        if (_this.toolbar) {
            if (_this.toolbarWay == "contextmenu") {
                if (_this.toolbarLoad == "node") {
                    div += " d-contextmenu='true'>";
                }
                if (_this.toolbarLoad == "noleaf") {
                    if (!last) {
                        div += " d-contextmenu='true'>";
                    } else {
                        div += " d-contextmenu='false'>";
                    }
                }
    ...
    DTree.prototype.toolbarListener = function (tool, $div) {
        ...
        switch (tool) {
            ...
            case defaultTool.delTool:
                layer.confirm('确定要删除该' + _this.toolbarStyle.title + '？', {
                    icon: 3,
                    title: '删除' + _this.toolbarStyle.title
                }, function (index) {
        ...
        switch (name) {
            case defaultTool.addTool:
                var addNodeBarDef = [{
                    ...
                    {
                        "label": "新增" + _this.toolbarStyle.title,
                        "name": "addNodeName",
                        "type": "text",
                        "value": "",
                        "defElem": "nowChange",
                    ...
                    },
                    {
                        ...
                        "value": "确认添加",
                        "defElem": "btn",
                        "filter": "dtree_addNode_" + _this.obj[0].id + "_form"
                    }];
            ...
            case defaultTool.editTool:
                var editNodeBarDef = [{
                    ...
                    {
                        "label": "编辑" + _this.toolbarStyle.title,
                        "name": "editNodeName",
                        "type": "text",
                        "value": "",
                        "defElem": "nowChange",
                    ...
                    },
                    {
                        ...
                        "value": "确认编辑",
                        "defElem": "btn",
                        "filter": "dtree_editNode_" + _this.obj[0].id + "_form"
                    }];
    ...
    DTree.prototype.getParam = function (id) {
        ...
        var $div = (typeof id === 'object') ? id : (_this.obj.find("div[dtree-click='" + eventName.itemNodeClick + "'][data-id='" + id + "']").length == 0) ? null : _this.obj.find("div[dtree-click='" + eventName.itemNodeClick + "'][data-id='" + id + "']");
        if ($div != null) {
            return _this.callbackData().node(_this.getTempNodeParam($div));
        } else {
            return {};
        }
    };
    ...
    DTree.prototype.getParentParam = function (id) {
        ...
        var $div = (typeof id === 'object') ? id : (_this.obj.find("div[dtree-click='" + eventName.itemNodeClick + "'][data-id='" + id + "']").length == 0) ? null : _this.obj.find("div[dtree-click='" + eventName.itemNodeClick + "'][data-id='" + id + "']");
        if ($div != null) {
            return _this.callbackData().parentNode($div);
        } else {
            return {};
        }
    };
    ...
    DTree.prototype.getChildParam = function (id) {
        ...
        var $div = (typeof id === 'object') ? id : (_this.obj.find("div[dtree-click='" + eventName.itemNodeClick + "'][data-id='" + id + "']").length == 0) ? null : _this.obj.find("div[dtree-click='" + eventName.itemNodeClick + "'][data-id='" + id + "']");
        if ($div != null) {
            return _this.callbackData().childNode($div);
        } else {
            return [];
        }
    };
    ...
    DTree.prototype.callbackData = function () {
        ...
        return {
            ...
            parentNode: function ($div) {	// 获取上级节点值
                ...
                if ($pdiv.length > 0) {
                    return _this.getRequestParam(_this.getTempNodeParam($pdiv));
                } else {
                    return {};
                }
